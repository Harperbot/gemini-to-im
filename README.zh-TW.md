...
> 想把 Google Gemini 打造成專屬於你的 AI 助理嗎？
>
> 我開源了一個名為 `gemini-to-im` 的小工具，它可以將 Gemini 連接到你的 Telegram，變成一個能**真正控制你電腦**的超強助理！
>
> 你可以透過手機，讓它幫你在台北找停車位、查詢墾丁的浪況，甚至執行任何你電腦裡的客製化腳本（需要你按鈕核准）。
>
> 而且它完全免費、開源、並將所有對話紀錄都存在你自己的電腦上，隱私滿分！今天就來打造你專屬的超能力 AI 助理吧！
>
> **🔗 GitHub 連結**: `https://github.com/Harperbot/gemini-to-im`
>
> `#Gemini #AI助理 #開源專案 #TelegramBot #在地化`
...
### 第三步：設定與執行
1. **下載程式碼**：點擊網頁右上角的綠色按鈕 `Code` -> `Download ZIP` 並解壓縮。
2. **開啟終端機**：Windows 搜尋 `cmd` (命令提示字元)，Mac 搜尋 `終端機` (Terminal)。
3. **進入資料夾**：在終端機輸入 `cd `（後面有一個空格），然後將您解壓縮後的資料夾**直接拖進**終端機視窗，按下 Enter。
4. **安裝必要套件**：輸入指令 `npm install` 並按下 Enter。*(如果跑出一堆警告訊息是正常的，只要沒有紅色的 Error 即可)*
5. **建立設定檔 (.env)**：
   - 在資料夾中新增一個文字檔。
   - 開啟它，貼上 `.env.example` 的內容並填入您的金鑰。
   - **另存新檔**，檔名輸入 `".env"` (包含引號，這能確保檔名正確且沒有後綴)。
6. **啟動機器人**：最後輸入 `node index.js` 即可啟動！之後只要對著您的 Telegram Bot 說話即可。
